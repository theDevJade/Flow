...
class AuthService {
    ...
    init {
        ...
        if (FlowConfiguration.webserverConfig.enableTestingAccount == true) {
    ...
    private fun createTestingAccount() {
        val testUsername = "admin"
        val testPassword = "admin123"


        val existingUser = getUserByUsername(testUsername)
        if (existingUser != null) {
            debugLog("Testing account already exists: $testUsername")
            return
        }


        val user = createUser(testUsername, testPassword)
        if (user != null) {
            debugLog("Created testing account: $testUsername with password: $testPassword")
        } else {
            debugLog("Failed to create testing account: $testUsername")
        }
    }
}