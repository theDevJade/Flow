...
pub fn execute(name: String, kind: String) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let package_type = match kind.as_str() {
        ...
        _ => {
            ...
            return Err(format!("Invalid package type '{}'. Use 'bin', 'lib', or 'native'", kind).into());
        }
    };
    ...
    println!("  {} Package type: {}", "✓".green().bold(), 
        match package_type {
            ...
            PackageType::Native => "native C library",
        }.yellow().bold());
    ...
    match package_type {
        ...
        PackageType::Native => {
            file_spinner.set_message("Creating src/lib.c...");
            thread::sleep(Duration::from_millis(300));
            let lib_c = src_dir.join("lib.c");
            fs::write(&lib_c, create_native_template(&pkg_name))?;
            file_spinner.finish_with_message(format!("{} Created src/lib.c", "✓".green().bold()));
            
            // Also create a header file
            let header_spinner = ProgressBar::new_spinner();
            header_spinner.set_style(
                ProgressStyle::default_spinner()
                    .template("  {spinner:.green} {msg}")
                    .unwrap()
                    .tick_strings(&["⣾", "⣽", "⣻", "⢿", "⡿", "⣟", "⣯", "⣷"])
            );
            header_spinner.enable_steady_tick(Duration::from_millis(80));
            header_spinner.set_message("Creating src/lib.h...");
            thread::sleep(Duration::from_millis(300));
            let lib_h = src_dir.join("lib.h");
            fs::write(lib_h, create_native_header_template(&pkg_name))?;
            header_spinner.finish_with_message(format!("{} Created src/lib.h", "✓".green().bold()));
        }
    }
...
"#, name, kind, 
    match package_type {
        PackageType::Bin => format!("Run the executable:\n```bash\n./{}\n```", name),
        PackageType::Lib => "Add as a dependency:\n```toml\n[dependencies]\n".to_string() + name + " = \"0.1.0\"\n```",
        PackageType::Native => "Add as a dependency in your Flow package:\n```toml\n[dependencies]\n".to_string() + name + " = \"0.1.0\"\n```\n\nThis native library can be used from Flow code through C interop.",
    })
}

fn create_native_template(name: &str) -> String {
    format!(r#"// {} - Native C library
// Generated by River package manager
...
fn create_native_header_template(name: &str) -> String {
    let guard = format!("{}_H", name.to_uppercase().replace("-", "_"));
    format!(r#"// {} - Native C library header
// Generated by River package manager
