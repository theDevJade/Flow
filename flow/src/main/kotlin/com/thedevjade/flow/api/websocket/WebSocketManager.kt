...
class WebSocketManager(
    ...
    private val webSocketProvider: WebSocketProvider? = null
) {
    ...
    suspend fun unregisterConnection(connectionId: String) = connectionMutex.withLock {
        ...
        if (connection != null) {
            ...
            eventManager.emit(
                WebSocketEvent.ConnectionClosed(
                    connectionId,
                    connection.userId,
    ...
    suspend fun sendMessage(
        ...
        return try {
            ...
            webSocketProvider?.sendMessage(connectionId, message) ?: false
        } catch (e: Exception) {
            eventManager.emit(
                WebSocketEvent.ConnectionError(
                    connectionId,
    ...
    suspend fun broadcastMessage(
        ...
        return webSocketProvider?.broadcastMessage(message, excludeConnectionId) ?: run {
            var sentCount = 0
    ...
    suspend fun sendMessageToUser(
        ...
        return webSocketProvider?.sendMessageToUser(userId, message) ?: run {
    ...
    private fun registerDefaultHandlers() {
        registerMessageHandler("auth") { context, data ->
            ...
            if (userId != null && username != null) {
                when (val authResult = userManager.authenticateUser(userId, context.connectionId, username)) {
...
interface WebSocketProvider {
    suspend fun sendMessage(connectionId: String, message: WebSocketMessage): Boolean
    suspend fun broadcastMessage(message: WebSocketMessage, excludeConnectionId: String? = null): Int
    suspend fun sendMessageToUser(userId: String, message: WebSocketMessage): Int
}