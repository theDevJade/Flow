...
class WorkspaceTopBar extends StatelessWidget {
  ...
  void _showCreateWorkspaceDialog(
    ...
    showDialog(
      context: context,
      builder: (context) => _CreateWorkspaceDialog(manager: manager),
    );
  }
}
...
class _CreateWorkspaceDialog extends StatefulWidget {
  ...
  const _CreateWorkspaceDialog({required this.manager});
...
class _CreateWorkspaceDialogState extends State<_CreateWorkspaceDialog> {
  ...
  void initState() {
    ...
    widget.manager.addListener(_onWorkspaceManagerChanged);
  }

  @override
  void dispose() {
    widget.manager.removeListener(_onWorkspaceManagerChanged);
    _nameController.dispose();
    super.dispose();
  }

  void _onWorkspaceManagerChanged() {
  ...
  Widget build(BuildContext context) {
    return AlertDialog(
      ...
      actions: [
        TextButton(
          onPressed: widget.manager.isLoading
              ? null
              : () {
                  widget.manager.clearError();
                  Navigator.of(context).pop();
                },
          ...
          onPressed: widget.manager.isLoading
              ...
              : () {
                  if (_nameController.text.trim().isNotEmpty) {
                    widget.manager.createWorkspace(_nameController.text.trim());

                  }
                },
          child: widget.manager.isLoading
              ? const SizedBox(
                  width: 16,
                  height: 16,
