...
class GraphEditor extends StatefulWidget {
  ...
  final Function(bool, String?)? onLoadingStateChanged; // Callback for loading state changes
...
class _GraphEditorState extends State<GraphEditor>
  ...
  void initState() {
    ...
    try {
      ...
      debugPrint('GraphEditor: Error initializing WebSocket service: $e');
    ...
    try {
      ...
      debugPrint('GraphEditor: Error initializing persistence service: $e');
    }

    // Load node templates if not already loaded
    try {
      if (!NodeTemplateService.instance.isLoaded) {
        debugPrint('GraphEditor: Node templates not loaded, loading now...');
        NodeTemplateService.instance.loadTemplates();
      } else {
        debugPrint('GraphEditor: Node templates already loaded');
      }
    } catch (e) {
      debugPrint('GraphEditor: Error loading node templates: $e');
    }
    ...
    try {
      ...
      debugPrint('GraphEditor: Error loading graph from persistence service: $e');
    }
  ...
  void _loadGraphFromPersistenceService() {
    ...
    // Show loading indicator (defer to next frame to avoid setState during build)
    WidgetsBinding.instance.addPostFrameCallback((_) {
      widget.onLoadingStateChanged?.call(true, 'Loading graph...');
    });
    ...
    if (cachedData != null) {
      ...
      Timer(const Duration(seconds: 3), () {
        if (mounted && nodes.isEmpty) {
          ...
          WidgetsBinding.instance.addPostFrameCallback((_) {
            widget.onLoadingStateChanged?.call(true, 'Loading from local storage...');
          });
  ...
  */
  ...
  void _addNodeFromTemplate(NodeTemplate template, Offset position) {
    ...
    // Auto-scale the node height based on ports
    newNode.autoScaleHeight();
  ...
  void _handleScaleUpdate(ScaleUpdateDetails details) {
    ...
    if (details.scale != 1.0) {
      ...
      if ((newScale - scale).abs() > 0.001) {
        // Calculate the point to zoom towards (the focal point)
        final graphPoint = _screenToGraph(details.localFocalPoint);
        final newPanOffset = details.localFocalPoint - (graphPoint * newScale);
      ...
      if (isDraggingNode && draggingNodeId != null && dragStartOffset != null) {
        ...
        if (_lastPanUpdate == null ||
          ...
          // Reduced to 8ms for smoother panning on macOS
          _lastPanUpdate = now;
          ...
          if ((constrainedPanOffset - panOffset).distance > 0.1) {
            // Reduced threshold for smoother panning
            setState(() {
              panOffset = constrainedPanOffset;
            });
            // Don't save view state on every pan update - only on pan end
          }
        }
      }
      lastFocalPoint = details.localFocalPoint;
  ...
  void _handlePanUpdate(DragUpdateDetails details) {
    ...
    if (!isDraggingNode && !isConnecting && lastFocalPoint != null) {
      ...
      // Update pan offset for smooth panning with reduced threshold
      if ((constrainedPanOffset - panOffset).distance > 0.1) {
        setState(() {
          panOffset = constrainedPanOffset;
        });
  ...
  Offset _screenToGraph(Offset screenPosition) {
    return coordinateSystem.screenToGraph(screenPosition);
  }
