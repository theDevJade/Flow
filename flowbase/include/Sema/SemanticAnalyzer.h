...
public:
    ...
    void define(const std::string& name, std::shared_ptr<Type> type, bool isMutable = false, bool isFunction = false);
...
private:
    ...
    void importSymbolsFrom(std::shared_ptr<Program> module, const std::vector<std::string>& symbols, const std::string& alias);
