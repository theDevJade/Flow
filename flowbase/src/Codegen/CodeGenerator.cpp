...
void CodeGenerator::declareBuiltinFunctions() {
    ...
    llvm::Function::Create(
        llvm::FunctionType::get(llvm::PointerType::get(*context, 0), 
            {llvm::PointerType::get(*context, 0), llvm::Type::getInt32Ty(*context), llvm::Type::getInt32Ty(*context)}, false),
        ...
        case TokenType::NOT:
            ...
            if (operand->getType()->isIntegerTy(1)) {
                ...
                llvm::Value* boolVal = builder->CreateICmpNE(operand, 
                    llvm::ConstantInt::get(*context, llvm::APInt(32, 0)), "tobool");
...
void CodeGenerator::visit(IndexExpr& node) {
    ...
    if (lengthIt != arrayLengths.end()) {
        ...
        // Check if index < 0
        llvm::Value* isNegative = builder->CreateICmpSLT(indexValue, llvm::ConstantInt::get(*context, llvm::APInt(32, 0)), "isneg");
