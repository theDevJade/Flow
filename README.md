...
For development and testing purposes, you can enable a default testing account by setting `enableTestingAccount = true` in your configuration file (`config.toml`):

```toml
enableTestingAccount = true
```

This will automatically create a testing account with:
- **Username**: `admin`
- **Password**: `admin123`

The account is only created if it doesn't already exist, and the flag can be safely left enabled in production (it won't create duplicate accounts).
